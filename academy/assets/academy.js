...
const K=path=>new URL(`../../knowledge/${path}`,import.meta.url);
...
export function bootFamilyStatic(){chrome();}
export async function bootFamily(){
 chrome();
 const id=document.body.dataset.familyId||new URLSearchParams(location.search).get('id')||'GLAMPING';const snap=await j(K('iskra_public_snapshot.json'));const f=snap.families.find(x=>x.id===id);
 ...
 const rail=document.querySelector('#family-rail');if(rail)rail.innerHTML=snap.families.filter(x=>x.id!==id).map(x=>`<a class="family-rail-item family-${slug(x.id)}" href="${familyHref(x.id)}"><span>${esc(x.id)}</span><b>${esc(x.title)}</b><i>→</i></a>`).join('');
}
export async function bootTraining(){
 chrome();const [c,g]=await Promise.all([j(K('curriculum/curriculum.json')),j(K('curriculum/glossary.json'))]);
...
export async function bootStart(){
 chrome();const g=await j(K('project_start_guides.json'));const u=g.universal_start;
...
export async function bootBriefBuilder(){
 chrome();
 const snap=await j(K('iskra_public_snapshot.json')),form=document.querySelector('#brief-form'),family=document.querySelector('#brief-family');
