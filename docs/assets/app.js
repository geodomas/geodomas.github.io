...
async function run(form,out,fn){if(!form||!out)return;form.addEventListener('submit',async e=>{e.preventDefault();setOutput(out,'Working…');try{setOutput(out,await fn())}catch(err){setOutput(out,`Request rejected: ${err.message}`,false)}})}
...
async function bootKnowledge(){
  try{
    const d=await loadProductOntology(); const el=$('#knowledge-output'); if(!el)return;
    const lines=d.families.map(f=>`${f.id} — ${f.name}\n${f.what_it_is}\nBest for: ${f.best_for.join(', ')}\nDiffers: ${f.how_it_differs}`).join('\n\n');
    setOutput(el,lines);
    const g=await loadProjectGuide(); const pe=$('#project-output'); if(pe)setOutput(pe,g.universal_start.steps.map(x=>`${x.step}. ${x.name} — ${x.ask.join(' / ')}`).join('\n'));
