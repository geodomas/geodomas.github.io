...
function modelLabel(m){
  ...
  let a=[m.id||m.name]; if(m.diameter_m) a.push(`Ø${m.diameter_m} m`); if(m.area_m2) a.push(`${m.area_m2} m²`); return a.filter(Boolean).join(' · ');
}
export async function bootIndex(){
 ...
 grid.innerHTML=snap.families.map(f=>`<a class="card" href="family.html?id=${encodeURIComponent(f.id)}"><span class="tag">${esc(f.id)} · ${esc(f.authority)}</span><h3>${esc(f.title)}</h3><p>${esc(f.summary)}</p><div class="chips">${(f.best_for||[]).slice(0,3).map(x=>`<span>${esc(x)}</span>`).join('')}</div><div class="more">Explore family →</div></a>`).join('');
 ...
 document.querySelector('#logic').innerHTML=snap.logic.selection_order.map(x=>`<span>${esc(x)}</span>`).join('');
}
export async function bootFamily(){
 const id=new URLSearchParams(location.search).get('id')||'GLAMPING'; const snap=await j('../knowledge/iskra_public_snapshot.json');
 const f=snap.families.find(x=>x.id===id); if(!f){document.querySelector('main').innerHTML='<p>Unknown family.</p>';return;}
 document.title=`${f.title} · GEODOMAS Academy`; document.querySelector('#fid').textContent=f.id; document.querySelector('#title').textContent=f.title; document.querySelector('#summary').textContent=f.summary; document.querySelector('#difference').textContent=f.difference;
 ...
 document.querySelector('#start').innerHTML=(f.project_start||[]).map((x,i)=>`<li>${i+1}. ${esc(x)}</li>`).join('');
 const models=familyModels(f); document.querySelector('#models').innerHTML=models.length?models.map(m=>`<div class="model"><strong>${esc(modelLabel(m))}</strong><small>${esc(typeof m==='object'?(m.program||m.role||m.status||m.release||m.maturity||'orientation'):'orientation')}</small></div>`).join(''):'<p class="note">This family is routed by diameter/configuration rather than a single public model list.</p>';
...
export async function bootTraining(){
 const [c,g]=await Promise.all([j('../knowledge/curriculum/curriculum.json'),j('../knowledge/curriculum/glossary.json')]);
 document.querySelector('#modules').innerHTML=c.modules.map(m=>`<article class="card"><span class="tag">${esc(m.id)} · ${esc(m.level)}</span><h3>${esc(m.title)}</h3><p>${esc((m.objectives||[]).join(' · '))}</p><div class="more">Pass: ${esc(m.pass_condition||'')}</div></article>`).join('');
...
export async function bootStart(){
 const g=await j('../knowledge/project_start_guides.json'); const u=g.universal_start;
 document.querySelector('#steps').innerHTML=u.steps.map(x=>`<article class="card"><span class="tag">STEP ${String(x.step).padStart(2,'0')}</span><h3>${esc(x.name)}</h3><p>${esc(x.ask.join(' · '))}</p></article>`).join('');
